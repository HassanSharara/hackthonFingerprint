...
# ============================================================
class FingerprintRectifier(nn.Module):
...
class RectificationTrainer:
    ...
    def train(self, epochs=20, batch_size=16, save_every=500):
        ...
        model = FingerprintRectifier().to(device)
    ...
    def matchingFingerprint(self, image_path, folder_to_lookat="../SOCOFing/Real"):
        ...
        best_match = None
        best_score = -1
        # Compare against all real images
        for f in os.listdir(folder_to_lookat):
            fpath = os.path.join(folder_to_lookat, f)
            db_img = cv2.imread(fpath, cv2.IMREAD_GRAYSCALE)
            if db_img is None: 
                continue
            db_img = cv2.resize(db_img, (128, 128))
            ...
            if score > best_score:
                best_score = score
                best_match = f
        return best_match, (best_score/400) * 100
